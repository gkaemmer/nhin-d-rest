...
class MessagesControllerTest < ActionController::TestCase
  ...
  should 'allow not access to POST /messages for unencrypted messaages for hisp role' do
    @request.env['HTTP_SSL_CLIENT_VERIFY'] = 'SUCCESS'
  ...
  should 'allow hisp to POST encrypted message' do
    @request.env['HTTP_SSL_CLIENT_VERIFY'] = 'SUCCESS'
    @request.headers['Content-Type'] = 'message/rfc822'
    @request.accept = 'message/rfc822'
    RemoteHISP.any_instance.stubs(:certs).returns [OpenSSL::X509::Certificate.new(TO_CRT)]
    post :create, {:domain => 'nhin.happyvalleypractice.example.org', :endpoint => 'drjones', :message => {:raw_message =>  ENCRYPTED_MESSAGE} }
